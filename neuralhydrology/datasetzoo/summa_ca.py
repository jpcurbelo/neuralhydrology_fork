...
def load_summa_ca_forcings(data_dir: Path, basin: str) -> Tuple[pd.DataFrame, int]:
    ...
    # forcing_path = os.path.join(data_dir, 'summa_input-output_LSTM_1D')
    forcing_path = data_dir / 'summa_input-output_LSTM_1D'
    
    try:
        # file_path = os.path.join(forcing_path, f'{basin}_forcing_leap_inp-out.csv')
        file_path = forcing_path / f'{basin}_forcing_leap_inp-out.csv'
    except:
